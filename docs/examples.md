...
同一套业务函数先本地校验，再经官方 MCP SDK 被 Client 发现。`stdio` 和 `http`（默认 `http://127.0.0.1:3333/mcp`）可供 Cursor / Claude 接入。HTTP 可配 Bearer token，角色由 token 决定；reader 看不到 write 能力。
