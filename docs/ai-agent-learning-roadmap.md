...
**预计学习时间**: 4-6 个月（每周 10-15 小时）。如果目标只是交付一个 AI 应用，可跳过训练实现，保留模型推理、RAG、评测和安全部分。

### 当前路线的补全专题

原路线偏重框架和 Agent API。新增内容按“模型 → 数据 → 可靠性 → 协议”补齐：

| 阶段 | 文档 | 目标 |
|------|------|------|
| 2.5 | [LLM 工程基础](./28-llm-engineering-foundations.md) | 理解训练、推理、微调和模型适配 |
| 3.5 | [RAG 数据管道与评测](./29-rag-data-and-evaluation.md) | 能定位召回问题并建立指标 |
| 4.5 | [Agent 可靠性与安全](./30-agent-reliability-and-security.md) | 能控制副作用、权限和失败恢复 |
| 5.5 | [MCP 与 Agent 协议](./31-mcp-and-agent-protocols.md) | 能把工具能力标准化并复用 |
