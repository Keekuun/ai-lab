...
词项检索（bigram + BM25 词频饱和）+ 按标题分块 + K=5 的基线（测试里设了防退化阈值）：
...
分词演进：连续中文整段成 token（Recall@5 = 0.53，「薪资」打不中「薪资计算」）→ bigram 二字滑动窗口（0.83）→ 再加 BM25 词频饱和 `tf/(tf+k1)` 防长文霸榜（拒答率 0.73 → 0.93）。改检索器看指标动，正是 29 的调参方法。
